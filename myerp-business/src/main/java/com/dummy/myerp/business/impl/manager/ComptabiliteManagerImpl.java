...
public class ComptabiliteManagerImpl extends AbstractBusinessManager implements ComptabiliteManager {
    ...
    public synchronized void addReference(EcritureComptable pEcritureComptable) throws FunctionalException {
        ...
        /* Le principe :
                ...
                l'année de l'écriture (table sequence_ecriture_comptable)*/
        SimpleDateFormat sdf = new SimpleDateFormat("yyyy");
        year = Integer.parseInt(sdf.format(pEcritureComptable.getDate()));

        SequenceEcritureComptable vSEC = getDaoProxy().getComptabiliteDao().
                getLastSeqOfTheYear(year, pEcritureComptable.getJournal().getCode());
        ...
        seq.setAnnee(year);
        seq.setDerniereValeur(recordNb);

                /*3.  Mettre à jour la référence de l'écriture avec la référence calculée (RG_Compta_5)*/

        String updateRefEntry = pEcritureComptable.getJournal().getCode() +
                                "-" + year + "/" + String.format("%05d", recordNb);

        pEcritureComptable.setReference(updateRefEntry);
    ...
    protected void checkEcritureComptableUnit(EcritureComptable pEcritureComptable) throws FunctionalException {
        ...
        SimpleDateFormat sdf = new SimpleDateFormat("yyyy");
        String anneeRef = sdf.format(pEcritureComptable.getDate());

        //Pour l'annee
        if(!anneeRef.equals(pEcritureComptable.getReference().substring(3,7))){
            throw new FunctionalException("La date de la reference ne " +
                    "correspond pas a la date de l'ecriture");
        }

        //Pour le code journal
        if (!pEcritureComptable.getJournal().getCode().equals(pEcritureComptable.getReference().substring(0,2))){
            throw new FunctionalException("Le code journal ne correspond pas " +
                    "a celui de l'ecriture comptable correspondante");
        }
    ...
    public void insertEcritureComptable(EcritureComptable pEcritureComptable) throws FunctionalException {
        ...
        try {
            ...
            getTransactionManager().rollbackMyERP(vTS);
                    ...
                    //compare ces 2 methodes et verifie si unes des 2 deconne
    /**
     * {@inheritDoc}
     */
    @Override
